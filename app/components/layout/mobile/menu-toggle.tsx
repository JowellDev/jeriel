import * as React from 'react'
import { motion, type SVGMotionProps } from 'framer-motion'

interface PathProps extends SVGMotionProps<SVGPathElement> {}

const Path: React.FC<PathProps> = props => (
	<motion.path
		fill="transparent"
		strokeWidth="3"
		stroke="hsl(175, 50%, 40%)"
		strokeLinecap="round"
		{...props}
	/>
)

interface MenuToggleProps {
	toggle: () => void
}

export const MenuToggle: React.FC<MenuToggleProps> = ({ toggle }) => (
	<button id="menu-button" onClick={toggle}>
		<svg width="23" height="23" viewBox="0 0 23 23">
			<Path
				variants={{
					closed: { d: 'M 2 2.5 L 20 2.5', stroke: 'hsl(175, 50%, 40%)' },
					open: { d: 'M 3 16.5 L 17 2.5', stroke: 'hsl(0, 0%, 100%)' },
				}}
			/>
			<Path
				d="M 2 9.423 L 20 9.423"
				variants={{
					closed: { opacity: 1, stroke: 'hsl(175, 50%, 40%)' },
					open: { opacity: 0, stroke: 'hsl(0, 0%, 100%)' },
				}}
				transition={{ duration: 0.1 }}
			/>
			<Path
				variants={{
					closed: { d: 'M 2 16.346 L 20 16.346', stroke: 'hsl(175, 50%, 40%)' },
					open: { d: 'M 3 2.5 L 17 16.346', stroke: 'hsl(0, 0%, 100%)' },
				}}
			/>
		</svg>
	</button>
)
