...
const SpeedDialItem: React.FC<Readonly<SpeedDialItemProps>> = ({
	...
	>
		<button
			onClick={onClick}
			className="p-3 bg-primary text-white rounded-full shadow-lg transition-colors flex gap-1 items-center"
		>
			<Icon size={24} />
...
> = ({ items, onClick }) => {
	...
	return (
		<div className="fixed bottom-5 right-5 flex flex-col items-end sm:hidden">
			...
			<button
				onClick={toggleMenu}
				className={`p-6 bg-primary text-white rounded-full shadow-lg transition-colors ${isOpen && 'rotate-45'}`}
			>
				<RiAddLine size={24} />
			</button>
		</div>
